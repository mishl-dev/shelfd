...
pub fn load_config(args: &ServeArgs) -> Result<AppConfig> {
    Ok(AppConfig {
        ...
        search_result_limit: parse_env("SEARCH_RESULT_LIMIT", 25)?,
        explore_page_size: parse_env("EXPLORE_PAGE_SIZE", 50)?,
        cover_lookup_limit: parse_env("COVER_LOOKUP_LIMIT", 8)?,
        inline_info_concurrency: parse_env("INLINE_INFO_CONCURRENCY", 8)?,
        cover_lookup_concurrency: parse_env("COVER_LOOKUP_CONCURRENCY", 4)?,
        upstream_retry_attempts: parse_env("UPSTREAM_RETRY_ATTEMPTS", 3)?,
        upstream_retry_backoff_ms: parse_env("UPSTREAM_RETRY_BACKOFF_MS", 250)?,
